...
# ---------------------------------------------------------------------------
# 语义型上下文压缩计划（Phase 1：只生成计划，不改变压缩行为）
# ---------------------------------------------------------------------------

SEMANTIC_KEEP_SCORE = 80
SEMANTIC_STRUCTURE_SCORE = 60
SEMANTIC_TOOL_NOISE_TOKENS = 1_000

_FILE_REF_RE = re.compile(
    r"(?<![\w/.-])(?:[\w.-]+/)+[\w./-]+\.[A-Za-z0-9_]+"
    r"|(?<![\w.-])[\w.-]+\.(?:py|md|yaml|yml|toml|json|txt|ts|tsx|js|jsx|java|go|rs|c|cpp|h|hpp)"
)


@dataclass
class SemanticMessageMeta:
    index: int
    role: str
    tags: set[str]
    importance: int
...
@dataclass
class SemanticCompactPlan:
    keep_verbatim: list[int]
    structure_extract: list[int]
    summarize: list[int]
    drop: list[int]
    must_keep_facts: list[str]
    metas: list[SemanticMessageMeta]


def _message_semantic_text(msg: Message) -> str:
    parts: list[str] = []
    if msg.content:
        parts.append(msg.content)
    for tu in msg.tool_uses:
        parts.append(tu.tool_name)
        parts.append(json.dumps(tu.arguments, ensure_ascii=False))
...
def _semantic_reason(tags: set[str]) -> str:
    if not tags:
        return "no semantic signal"
    return ", ".join(sorted(tags))


def semantic_tag_message(
    msg: Message,
    index: int,
    total_messages: int,
) -> SemanticMessageMeta:
    """Classify one message for semantic compaction planning.

    This is deliberately heuristic and deterministic. It provides an auditable
    first layer before any LLM-assisted compaction is introduced.
    """
    text = _message_semantic_text(msg)
    lower = text.lower()
    tags: set[str] = set()
    token_estimate = _message_tokens(msg)
    file_refs = _extract_file_refs(text)
    tool_refs = [tu.tool_name for tu in msg.tool_uses]

    if msg.role == "user" and msg.content and not msg.content.startswith("<system-reminder>"):
        tags.add("user_goal")
    if any(
        marker in lower
        for marker in (
        ...
        tags.add("constraint")
    if index >= max(0, total_messages - MIN_KEEP_MESSAGES):
        tags.add("current_work")
    if file_refs or "```" in text:
        tags.add("code_fact")
    if any(marker in lower for marker in ("decision", "because", "reason", "设计", "选择", "理由", "方案")):
        tags.add("decision")
    if any(tr.is_error for tr in msg.tool_results) or any(
        marker in lower
        for marker in ("traceback", "exception", "error:", "failed", "失败", "报错", "错误")
    ):
        tags.add("error")
    if any(marker in lower for marker in ("todo", "next step", "pending", "下一步", "待办", "计划")):
        tags.add("todo")
    if msg.tool_results and token_estimate >= SEMANTIC_TOOL_NOISE_TOKENS:
        tags.add("tool_noise")
    if msg.role == "assistant" and msg.content and not msg.tool_uses:
        tags.add("chat")

    importance = 20
    if "user_goal" in tags:
        importance += 35
    if "constraint" in tags:
        importance += 35
    if "todo" in tags:
        importance += 25
    if "current_work" in tags:
        importance += 20
    if "code_fact" in tags:
        importance += 15
    if "decision" in tags:
        importance += 15
    if "error" in tags:
    ...
    if "chat" in tags:
        importance += 5
    if "tool_noise" in tags:
        importance -= 35
    if not text.strip():
        importance -= 10
    importance = max(0, min(100, importance))

    return SemanticMessageMeta(
        index=index,
        role=msg.role,
        tags=tags,
        ...
        file_refs=file_refs,
        tool_refs=tool_refs,
        reason=_semantic_reason(tags),
    )


def build_semantic_compact_plan(messages: list[Message]) -> SemanticCompactPlan:
    """Build an auditable semantic compaction plan for a conversation history."""
    metas = [
        semantic_tag_message(msg, index=i, total_messages=len(messages))
    ...
    for meta in metas:
        if "constraint" in meta.tags or "todo" in meta.tags or meta.importance >= SEMANTIC_KEEP_SCORE:
            keep_verbatim.append(meta.index)
        elif {"code_fact", "decision", "error"} & meta.tags and meta.importance >= SEMANTIC_STRUCTURE_SCORE:
            structure_extract.append(meta.index)
        elif "tool_noise" in meta.tags and meta.importance < SEMANTIC_STRUCTURE_SCORE:
            drop.append(meta.index)
        else:
            summarize.append(meta.index)

        if "constraint" in meta.tags:
            must_keep_facts.append(f"constraint@{meta.index}: {meta.reason}")
        if "todo" in meta.tags:
            must_keep_facts.append(f"todo@{meta.index}: {meta.reason}")
        for ref in meta.file_refs[:3]:
            must_keep_facts.append(f"file@{meta.index}: {ref}")

    return SemanticCompactPlan(
