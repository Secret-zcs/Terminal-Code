...
def _is_destructive_bash(command: str) -> bool:
    """Heuristic: does a Bash command look potentially destructive?"""
...
class Agent:
    def __init__(
        ...
        self.checkpoint_manager: Any = None  # CheckpointManager, set by app.py after init
            ...
            if not response.tool_calls:
                ...
                if self.checkpoint_manager is not None:
                    ...
                    self.checkpoint_manager.create_checkpoint(
                        label=f"Turn: {summary}",
                        trigger="turn_end",
                        conversation=conversation,
                        agent=self,
                    )
                elif self.file_history is not None:
                    # 旧路径（无 CheckpointManager 时回退）
                    summary = response.text[:60] + "..." if len(response.text) > 60 else response.text
            ...
            for batch in batches:
                if batch.concurrent and len(batch.calls) > 1:
                    ...
                    for tc in batch.calls:
                        ...
                        # ── 自动检查点：高风险工具执行前创建快照 ──
                        await self._maybe_auto_checkpoint(tc, conversation)
    ...
    async def _maybe_auto_checkpoint(
        self, tc: ToolCallComplete, conversation: ConversationManager
    ) -> None:
        """在高风险工具执行前自动创建检查点。

        由 checkpoint_manager 的速率限制器控制频率（默认最小间隔 30 秒）。
        manual 和 turn_end 触发类型不受速率限制。
        """
        ...
        if tool.category == "write":
            ...
            command = tc.arguments.get("command", "") if isinstance(tc.arguments, dict) else ""
            if command and _is_destructive_bash(command):
                trigger = "pre_bash"

        if trigger is None:
            return
        if not cm.should_auto_checkpoint(trigger):
            return

        label = f"Auto: before {tc.tool_name}"
