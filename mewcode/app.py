...
class MewCodeApp(App):
    ...
    def _select_provider(self, provider: ProviderConfig) -> None:
        ...
        # ── 初始化 CheckpointManager（快照/回退系统）──
        from mewcode.checkpoint import CheckpointManager
        checkpoint_dir = Path(work_dir) / ".mewcode" / "checkpoints" / self.session.session_id
        ...
        # 从磁盘恢复已有检查点
        loaded = self.checkpoint_manager.load_from_disk()
        if loaded > 0:
            log.info("Loaded %d checkpoints from disk", loaded)
        self.agent.checkpoint_manager = self.checkpoint_manager
