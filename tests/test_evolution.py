...
def _capture_self_evolution_match_mount(app) -> list[tuple[str, str, str]]:
    mounted: list[tuple[str, str, str]] = []
    ...
    async def fake_mount(
        ...
        mounted.append((match, audit, approval_request_id))
...
class TestEvolutionEngine:
    ...
    def test_render_skill_candidate_task_matches_summarizes_hidden_matches(
        self, tmp_path: Path
    ) -> None:
        engine = EvolutionEngine(tmp_path)
        ...
        assert "Shown matches: `1`" in markdown
        assert "Total matches: `2`" in markdown
        assert "Hidden matches: `1`" in markdown
    ...
    def test_self_evolution_review_notification_shows_completion_resume_failure(
        self,
    ) -> None:
        from mewcode.evolution.auto_review import format_review_notification
        from mewcode.evolution.models import SelfEvolutionReviewRun

        missing = format_review_notification({
            "status": "missing",
            "active_review_run_id": "review_missing",
            "requests": [],
            "blocked_generated_candidates": [],
        })
        not_running = format_review_notification({
            "status": "not-running",
            "review_run": SelfEvolutionReviewRun(
                id="review_done",
                mode="fork_reviewer",
                ...
                approval_mode="manual",
                artifacts={
                    "report": ".mewcode/evolution/review_runs/review_done/report.md",
                },
            ),
            "requests": [],
            "blocked_generated_candidates": [],
        ...
        assert "Self-evolution review run not found" in missing
        assert "review_missing" in missing
        assert "Self-evolution review run is not running" in not_running
        assert "review_done" in not_running
        assert "submitted" in not_running
    ...
    def test_self_evolution_review_trusted_auto_quarantines_after_new_negative_usage(
        self, tmp_path: Path
    ) -> None:
        from mewcode.config import SelfEvolutionConfig
        from mewcode.evolution.auto_review import review_ready_skill_candidates
        ...
        config = SelfEvolutionConfig(enabled=True, skill_approval_mode="trusted-auto")
        first = review_ready_skill_candidates(tmp_path, config)
        assert first["status"] == "auto-promoted"
        ...
        EvolutionEngine(tmp_path).record_skill_usage(
            ...
            event="failure",
            source="post-promote-test",
            metadata={"summary": "trusted-auto 后仍然遗漏验证。"},
        )

        second = review_ready_skill_candidates(tmp_path, config)

        assert second["status"] == "quarantined"
        assert second["auto_quarantines"][0]["skill_name"] == "review-loop"
        assert second["auto_quarantines"][0]["ok"] is True
        assert not promoted_path.exists()
        quarantine_path = Path(second["auto_quarantines"][0]["path"])
        assert quarantine_path.exists()
        usage = EvolutionEngine(tmp_path).load_skill_usage()
        assert usage[-1]["event"] == "quarantine"
        assert usage[-1]["metadata"]["source"] == "trusted-auto-rollback"

    def test_self_evolution_review_trusted_auto_uses_rollback_threshold(
        self, tmp_path: Path
    ) -> None:
        from mewcode.config import SelfEvolutionConfig
        from mewcode.evolution.auto_review import review_ready_skill_candidates
    ...
    def test_self_evolution_review_report_includes_generated_canary_summary(
        self, tmp_path: Path
    ) -> None:
        from mewcode.config import SelfEvolutionConfig
        from mewcode.evolution.auto_review import review_ready_skill_candidates
    ...
    def test_tui_self_evolution_review_does_not_show_empty_inbox(
        self, tmp_path: Path, monkeypatch: pytest.MonkeyPatch
    ) -> None:
        _install_fake_mcp(monkeypatch)
        import mewcode.app as app_module
    ...
    async def test_tui_user_message_shows_self_evolution_candidate_matches(
        ...
        match_markdown, audit_markdown, approval_request_id = mounted[0]
    ...
    async def test_tui_user_message_shows_only_top_self_evolution_candidate_match(
        ...
        assert messages == []
        assert len(mounted) == 1
        match_markdown, _audit_markdown, _approval_request_id = mounted[0]
    ...
    @pytest.mark.asyncio
    async def test_tui_user_message_match_card_can_open_pending_approval(
        self, tmp_path: Path, monkeypatch: pytest.MonkeyPatch
    ) -> None:
        _install_fake_mcp(monkeypatch)
        from mewcode.config import SelfEvolutionConfig
        ...
        await app._dispatch_command("修复复杂回归 bug 时先复现失败并写回归测试。")
        if app._agent_task is not None:
            await app._agent_task

        assert len(mounted) == 1
        match_markdown, _audit_markdown, approval_request_id = mounted[0]
    ...
    def test_tui_self_evolution_match_response_opens_pending_approval(
        self, monkeypatch: pytest.MonkeyPatch
    ) -> None:
        _install_fake_mcp(monkeypatch)
        from mewcode.self_evolution_dialog import (
    ...
    def test_tui_self_evolution_review_inlines_single_review_report(
        ...
        report_path = (
            ...
            / "evolution"
            / "review_runs"
            / "review_detail_source"
            / "report.md"
        )
        report_path.parent.mkdir(parents=True)
        report_path.write_text(
            "# Fork Reviewer Report\n\n候选 skill 通过了沙盒评测。\n",
            encoding="utf-8",
        )
        engine.store.save_self_evolution_review_run(
            SelfEvolutionReviewRun(
                id="review_detail_source",
                mode="fork_reviewer",
                status="generated",
                approval_mode="manual",
                artifacts={
                    "report": (
                        ".mewcode/evolution/review_runs/"
                        "review_detail_source/report.md"
                    )
                },
                summary={"generated_candidates": [proposal.id]},
                completed_at=1.0,
    ...
    def test_tui_self_evolution_review_omits_multiple_review_reports(
        ...
        engine = EvolutionEngine(tmp_path)
        first = engine.propose_skill(
            name="first-generated-loop",
            description="第一个等待 gate 的候选 skill",
            body="# 任务\n\n记录复盘步骤。\n",
        )
        second = engine.propose_skill(
            name="second-generated-loop",
            description="第二个等待 gate 的候选 skill",
            body="# 任务\n\n保留验证证据。\n",
        )
        for review_id, proposal_id, body in [
            ("review_first_source", first.id, "first report secret"),
            ("review_second_source", second.id, "second report secret"),
        ]:
            report_path = (
                tmp_path
                / ".mewcode"
            ...
            engine.store.save_self_evolution_review_run(
                SelfEvolutionReviewRun(
                    ...
                    artifacts={
                        "report": (
                            ".mewcode/evolution/review_runs/"
                            f"{review_id}/report.md"
                        )
                    },
                    summary={"generated_candidates": [proposal_id]},
                    completed_at=1.0,
                )
            )
        monkeypatch.setattr(
