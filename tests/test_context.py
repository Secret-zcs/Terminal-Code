...
# ---------------------------------------------------------------------------
# semantic compaction plan
# ---------------------------------------------------------------------------

class TestSemanticCompactPlan:
    def test_user_constraints_are_kept_and_recorded(self) -> None:
        msgs = [
            Message(role="user", content="必须形成 md 文档，并且每次修改都要留档。"),
            Message(role="assistant", content="我会先读取项目结构。"),
        ]

        plan = build_semantic_compact_plan(msgs)

        assert 0 in plan.keep_verbatim
        assert "constraint" in plan.metas[0].tags
        assert "user_goal" in plan.metas[0].tags
        assert any(fact.startswith("constraint@0") for fact in plan.must_keep_facts)

    def test_large_old_tool_result_is_drop_candidate(self) -> None:
        msgs = [
            Message(
                role="user",
                content="",
                tool_results=[
                    ToolResultBlock(
                        tool_use_id="toolu_big",
                        content="x" * int(1_200 * _CHARS_PER_TOKEN),
                    )
                ],
            ),
        ]
        ...
        assert 0 in plan.drop
        assert "tool_noise" in plan.metas[0].tags

    def test_recent_messages_are_kept_as_current_work(self) -> None:
        msgs = [Message(role="assistant", content=f"old message {i}") for i in range(6)]
        msgs.append(Message(role="assistant", content="下一步进入 mewcode/client.py。"))

        plan = build_semantic_compact_plan(msgs)

        assert 6 in plan.keep_verbatim
        assert "current_work" in plan.metas[6].tags
        assert "todo" in plan.metas[6].tags

    def test_file_refs_are_extracted_as_must_keep_facts(self) -> None:
        msgs = [
            Message(
                role="assistant",
                content="关键事实来自 mewcode/context/manager.py 和 docs/semantic-context-compression.md。",
            )
        ]

        plan = build_semantic_compact_plan(msgs)

        assert "code_fact" in plan.metas[0].tags
        assert "mewcode/context/manager.py" in plan.metas[0].file_refs
        assert any("docs/semantic-context-compression.md" in fact for fact in plan.must_keep_facts)

    def test_semantic_plan_protects_state_that_tail_window_misses(self) -> None:
        msgs = [
            Message(role="user", content="必须保留用户约束：每次修改都要留档。"),
            Message(role="assistant", content="已完成早期探索。"),
            Message(role="assistant", content="已完成第一轮模块讲解。"),
            Message(role="assistant", content="已完成第二轮模块讲解。"),
            Message(role="assistant", content="下一步计划：测试语义压缩前后效果。"),
            Message(
                role="user",
                content="",
                tool_results=[
                    ToolResultBlock(
                        tool_use_id="toolu_noise",
                        content="irrelevant log\n" * int(KEEP_RECENT_TOKENS * _CHARS_PER_TOKEN),
                    )
                ],
            ),
        ]

        old_keep_start = _compute_keep_start_index(msgs)
        old_tail_indices = set(range(old_keep_start, len(msgs)))
        semantic_plan = build_semantic_compact_plan(msgs)

        assert 0 not in old_tail_indices
        assert 4 not in old_tail_indices
        assert 5 in old_tail_indices

        assert 0 in semantic_plan.keep_verbatim
        assert 4 in semantic_plan.keep_verbatim
        assert 5 in semantic_plan.drop
        assert "constraint" in semantic_plan.metas[0].tags
        assert "todo" in semantic_plan.metas[4].tags
        assert "tool_noise" in semantic_plan.metas[5].tags
...
class TestBuildCompactMessages:
    def test_basic_structure(self) -> None:
        ...
        assert "上下文空间不足" in msgs[0].content
        assert "早期对话的摘要" in msgs[0].content
        assert "the summary" in msgs[0].content

    def test_includes_keep_tail_and_transcript_hints(self) -> None:
        msgs = build_compact_messages(
            "the summary",
            has_keep_tail=True,
            transcript_path=".mewcode/session/transcript.jsonl",
        )
        assert "近期消息已原样保留" in msgs[0].content
        assert "ReadFile" in msgs[0].content
        assert ".mewcode/session/transcript.jsonl" in msgs[0].content
