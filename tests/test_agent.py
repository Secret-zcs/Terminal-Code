...
async def test_multi_step_autonomous(tmp_path):
    ...
    file_path = str(tmp_path / "mewcode_test_hello.txt")
    client = MockLLMClient([
        # 第 1 轮：WriteFile
        [
...
async def test_message_splicing():
    ...
    # 检查对话历史；连续纯文本 user 消息会被合并，tool_result 批次保持独立。
    msgs = build_anthropic_messages(conv.get_messages())
    # user(env + prompt) + assistant(text+2 个 tool_use) + user(2 个 tool_result) + assistant(最终响应)
    assert len(msgs) == 4
