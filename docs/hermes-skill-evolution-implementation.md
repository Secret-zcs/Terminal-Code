...
当前实现已经覆盖 Hermes 运行时自进化的安全核心：学习入口、skill create、skill patch、candidate 隔离、验证、eval case、eval、execution eval report、sandbox artifacts、审批、promote、checkpoint、reload、usage log、手动 quarantine、隔离建议和 usage-driven patch candidate。与 Hermes 原版的差距主要在后台 fork review、真实沙盒任务回放 eval、自动 usage 归因和自动 patch 评审。
